...
cartApp.controller("addToCartCtrl", function ($scope, $http){
    ...
    $scope.removeItemFromCart = function (itemId) {
    	alert('remove called')
        $http.put('http://localhost:9080/musichub/usercart/cart/removeItem/'+itemId).success(function (data) {
    ...
    $scope.addItemToCart = function (itemId) {
        $http.put('http://localhost:9080/musichub/usercart/cart/addItem/'+itemId).success(function () {
    ...
    $scope.refreshCartItems = function () {
        $http.get('http://localhost:9080/musichub/usercart/cart/refreshCart/'+$scope.cartId).success(function (data) {
           $scope.cart=data;
        });
    };
